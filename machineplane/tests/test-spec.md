...
**content_hashing_is_deterministic**
- Given the same manifest content
- When computing the content hash multiple times
- Then the same digest is returned each time

### CLI Environment (cli_tests.rs)

**default_cli_values**
- Given no command-line arguments
- When parsing CLI configuration
- Then default values for REST API, Korium transport, and ephemeral keys are set correctly
...
**winner_selection_uses_identity_as_tiebreaker**
- Given bids with identical scores
- When selecting a winner
- Then the identity is used as tiebreaker
...
> Note: Integration tests run in CI with libkrun microVM runtime.
> Tests require libkrun to be installed on the system.
...
**apply_with_runtime_creates_and_removes_workloads**
- Given a manifest and running libkrun runtime
- When applying and then deleting the manifest
- Then microVMs are created and then cleaned up
